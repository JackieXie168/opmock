...
/* this test is ignored. You must put the comment start on the same line
than the signature, so that the signature does not match ^[\t ]*void
*/
...
void test_afizzbuzz_many_3_and_5()
{
  int i;
  for(i = 1; i < 1000; i++) {
    if((i % 3 == 0) && ((i % 5) == 0)) { 
